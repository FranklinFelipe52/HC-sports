  ...
  <div id="modal-excluir-inscricao-{{ $registration->id }}" class="hidden">
    <div class="flex h-screen w-full fixed bottom-0 bg-black bg-opacity-60 z-50 justify-center items-center">
      <div class="bg-white mx-3 p-3 md:p-6 rounded-lg w-full max-w-[532px]">
        ...
        <!-- modal footer - actions -->
        <div class="flex justify-end gap-4 flex-wrap mt-10">
          <a role="button" href="/inscricao/admin/registration/delete/{{ $registration->id }}" class="flex text-brand-v1 text-sm font-bold font-poppins items-center justify-center sm:justify-start gap-4 w-full sm:w-fit px-4 py-2.5 rounded border-[1.5px] border-brand-v1 hover:ring-2 hover:ring-brand-v1 hover:ring-opacity-50 bg-white transition">
            Excluir Inscrição
          </a>
      ...
      <div class="container h-full w-full flex flex-col overflow-auto pb-8">
        ...
        <header class="pt-8 pb-6 space-y-6">
          <nav aria-label="Breadcrumb" class="flex items-center flex-wrap gap-2">
            ...
            <div>
              <a href="/inscricao/admin/users/{{ $registration->user->id }}" class="text-xs text-gray-1 block hover:underline">
                ...
                <div class="col-span-2 sm:col-span-1">
                  <button class="inline-flex items-center gap-3 bg-fill-base p-2 pr-8 rounded-lg hover:bg-gray-6 transition">
                    <div class="w-[30px] h-[30px]">
                      <img src="/inscricao/images/svg/link.svg" class="h-full w-full object-cover" alt="">
                    </div>
            ...
            <div class="flex gap-4">
              @if (!($registration->type_payment_id == 2 && $registration->status_regitration_id == 1))
                @if ($registration->modalities_id == 9 || $registration->modalities_id == 10)
                  <a href="/inscricao/admin/registration/update/{{ $registration->id }}?gender={{ $registration->user->sexo }}" class="flex items-center justify-center w-fit text-sm font-bold text-brand-a1 p-2 px-4 rounded-md border border-brand-a1 hover:ring-2 hover:ring-brand-a1 hover:ring-opacity-50 disabled:hover:ring-0 disabled:opacity-50 disabled:cursor-not-allowed transition">
                    Editar
                  </a>

                @else
                  <a href="/inscricao/admin/registration/update/{{ $registration->id }}" class="flex items-center justify-center w-fit text-sm font-bold text-brand-a1 p-2 px-4 rounded-md border border-brand-a1 hover:ring-2 hover:ring-brand-a1 hover:ring-opacity-50 disabled:hover:ring-0 disabled:opacity-50 disabled:cursor-not-allowed transition">
                    Editar
                  </a>
                @endif
              @endif

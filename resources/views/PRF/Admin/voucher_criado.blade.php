...
@section('title', 'Gerar voucher - Meia Maratona PRF')
...
@section('content')
      ...
      <div class="flex flex-col h-full">
        ...
        <div class="p-8 pb-12 lg:p-8 my-auto">
          ...
          <p class="text-sm font-normal text-gray-1">
            Agora você pode copiar o voucher criado
          </p>
        </div>
          ...
          <div class="space-y-4 mb-6">
            <div class="group @error('descricao') error @enderror">
              <div class="relative">
                ...
                <input disabled value="KLJDAICAKCEAQEFJCA" class="disabled:bg-gray-6 disabled:cursor-not-allowed w-full px-4 py-3 rounded-lg border border-gray-4 focus:border-brand-prfA1 focus:outline-brand-prfA1 group-[.error]:border-input-error group-[.error]:outline-input-error text-gray-1 placeholder:text-gray-3 transition" type="text" id="voucher_field" name="voucher" placeholder="Código do voucher" />
              </div>
            </div>
          </div>
          <button id="copiar_button" onclick="copiarVoucher()" type="submit" class="flex items-center justify-center gap-4 w-full px-4 py-2.5 rounded border-[1.5px] border-brand-prfA1 hover:ring-2 hover:ring-brand-prfA1 hover:ring-opacity-50 bg-brand-prfA1 disabled:bg-gray-4 disabled:border-gray-4 disabled:hover:ring-0 disabled:cursor-not-allowed transition">
  ...
  <script>
    function copiarVoucher() {
      const voucherValue = document.getElementById("voucher_field").value;

      const tempInput = document.createElement("textarea");
      tempInput.value = voucherValue;
      document.body.appendChild(tempInput);
      tempInput.select();
      document.execCommand("copy");
      document.body.removeChild(tempInput);
      showSuccessToastfy("Voucher copiado para a área de transferência");
    }

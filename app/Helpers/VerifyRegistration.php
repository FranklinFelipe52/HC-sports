...
class VerifyRegistration
{
    ...
    public static function verifyModalitiesMinYear($category, $data_nasc, $modalidade, $federativeUnit)
    {
        $qLivre = 0;
        $registrations = [];
        switch ($category->id) {
            case 5:
                ...
                foreach ($registrations as $value) {
                    $idade = AgeBetweenDates::calc_idade($value->user->data_nasc, $modalidade->limit_year_date);
                    error_log('idade: idade');
                    error_log($idade);
                    if ($idade < $category->min_year) {
                        $qLivre++;
                    }
                }
                if (!($qLivre > 0)) {
            ...
            case 9:
                ...
                foreach ($registrations as $value) {
                    ...
                    if (($idade < $category->min_year) && (35 <= $idade)) {
                        $qLivre++;
                    }
                }
                if (!($qLivre > 0)) {
            ...
            case 10:
                ...
                foreach ($registrations as $value) {
                    ...
                    if (($idade < $category->min_year) && (45 <= $idade)) {
                        $qLivre++;
                    }
                }
                if (!($qLivre > 0)) {
